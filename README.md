...
## Latest fixes
- Direct HTTP/HTTPS downloads use Android DownloadManager and save to Downloads.
- Blob/data URL downloads generated by JavaScript are captured by a native Android JS bridge and saved to Downloads in chunks.
- Legacy Android devices (API <= 28) request storage permission when needed.
- Custom app icon generates legacy density icons plus an Android adaptive launcher icon.
- Custom splash image is copied into the native splash and is used as a fallback from the custom app icon.
- Splash/theme has a premium gradient, centered logo card, title/tagline and loading indicator.
